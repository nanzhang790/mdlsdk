...
// examples/example_distilling_GLSL.cpp
//
    ...
    const char* texture_name)
{
    ...
    return adjust_canvas(image_api, canvas, texture->get_effective_gamma());
}
    ...
    GLuint h)
{
    ...
    return state.mdl_compiler->export_canvas(filename, canvas.get());
}
...
private:
    ...
    void add_texture_runtime()
    {
        ...
        m_code +=
            "vec4 tex_lookup_2d("
            "uint tex, vec2 coord, int wrap_u, int wrap_v, vec2 crop_u, vec2 crop_v)\n"
            "{\n"
        ...
        for (mi::Size i = 1; i < m_target_code->get_texture_count(); ++i)
        {
            ...
            w_switch += "    case " + to_string(i) + "u: return "
                + to_string(image->resolution_x()) + ";\n";
            h_switch += "    case " + to_string(i) + "u: return "
                + to_string(image->resolution_y()) + ";\n";
        }
        m_code +=
            "int tex_width(uint tex, ivec2 uv_tile)"
            "{\n"
            "    switch (tex) {\n"
            "    case 0u: return 0;\n"
            + w_switch +
            "    }\n"
            "}\n"
            "int tex_height(uint tex, ivec2 uv_tile)"
            "{\n"
            "    switch (tex) {\n"
            "    case 0u: return 0;\n"
            + h_switch +
...
static GLuint prefilter_glossy(GLsizei w, GLsizei h, GLuint env_tex_id, GLuint env_accel_tex_id)
{
    ...
    Shader_program program(
        ...
        get_executable_folder() + "/" + "prefilter_glossy.frag");
...
static GLuint prefilter_diffuse(GLsizei w, GLsizei h, GLuint env_tex_id, GLuint env_accel_tex_id)
{
    ...
    Shader_program program(
        get_executable_folder() + "/" + "screen_aligned_quad.vert",
        get_executable_folder() + "/" + "prefilter_diffuse.frag");
    Screen_aligned_quad quad;
    quad.bind_shader(&program);
...
{
    Shader_program program(
        get_executable_folder() + "/" + "integrate_brdf.vert",
        get_executable_folder() + "/" + "integrate_brdf.frag");
    Screen_aligned_quad quad;
    GLuint fbo=0, rbo=0;
...
static GLFWwindow *createGLWindow(const Options& options, Window_context &window_context)
{
...
static void filterEnvTexture(
    const Mdl_sdk_state& state, const Options& options, GLuint &env_tex_id, GLuint &iblmap_id, GLuint &refmap_id, GLuint &brdflutid)
{
    env_tex_id = iblmap_id = refmap_id = brdflutid = 0;
    ...
    const Options& options)
{
    ...
    GLFWwindow *window = createGLWindow(options, window_context);
    ...
    filterEnvTexture(state, options, env_tex_id, iblmap_id, refmap_id, brdflutid);

    // Create scene data
    std::vector<Mdl_pbr_shader*> pbr_shaders(distilled_materials.size() * 2, nullptr);
    const std::string exefolder = get_executable_folder();
    Shader_program env_shader(exefolder + "/screen_aligned_quad.vert", exefolder + "/environment_sphere.frag");
    Screen_aligned_quad quad;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!sphere_shader) {
            const char *mode = (window_context.bake ? "baked" : "GLSL");
            std::cout << "Generating shader " << window_context.material << " in " << mode << " mode...\n";
            auto &a = distilled_materials[window_context.material];
            auto mdl_ue4 = window_context.bake ?
                (Mdl_ue4*)(new Mdl_ue4_baker(state, a.get(), options.baking_resolution_x, options.baking_resolution_y)) :
                (Mdl_ue4*)(new Mdl_ue4_glsl(state, a.get()));
        ...
        theta = std::min(std::max(theta, 0.025 * M_PI), 0.975 * M_PI);
        window_context.move_dx = window_context.move_dy = 0;
        ...
        const mi::Float32_4_4 proj = projection(45.f, float(window_context.width) / window_context.height, 1.f, 100.f);
