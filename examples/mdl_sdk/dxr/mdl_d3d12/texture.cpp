...
namespace mdl_d3d12
{
    ...
    {
        if (!has_flag(m_gpu_access, GPU_access::unorder_access))
        {
    ...
    bool Texture::create()
    {
        ...
        else
        {
            ...
            if (has_flag(m_gpu_access, GPU_access::depth_stencil_target))
            {
                resource_desc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
                m_latest_scheduled_state = D3D12_RESOURCE_STATE_DEPTH_WRITE;
            }

            if (has_flag(m_gpu_access, GPU_access::render_target))
            {
