...
namespace mdl_d3d12
{
        ...
        , m_exports(exported_symbols.size())
    {
        ...
        {
            m_exported_symbols[i] = str_to_wstr(exported_symbols[i]);
            m_exports[i] = {};
    ...
    Raytracing_pipeline::Hitgroup::Hitgroup(
        ...
        : m_name(str_to_wstr(name))
        , m_closest_hit_symbol(str_to_wstr(closest_hit_symbol))
        , m_any_hit_symbol(str_to_wstr(any_hit_symbol))
        , m_intersection_symbol(str_to_wstr(intersection_symbol))
    {
        ...
        , m_desc {}
    {
        ...
        {
            m_symbols[i] = str_to_wstr(symbols[i]);
        ...
        const std::vector<std::string>& exported_symbols)
    {
        ...
        for (const auto& s : lib.m_exported_symbols)
        {
            ...
            {
                log_error("Tried to add duplicated symbol '" + wstr_to_str(s) + 
                          "' to pipeline:" + m_debug_name + ".", SRC);
        ...
        std::string intersection_symbol)
    {
        ...
        std::wstring wname = str_to_wstr(name);
        ...
        for (auto&& s : group_symbols)
            if (m_all_exported_symbols.find(*s) == m_all_exported_symbols.end())
            {
                log_error("Tried to add non existing symbol '" + wstr_to_str(*s) + 
                          "' to hit-group '" + name + 
        ...
        const std::vector<std::string>& symbols)
    {
        ...
        // make sure the symbol (or hitgroup) to associate is available
    ...
    bool Raytracing_pipeline::finalize()
    {
        ...
        for (const auto& name : m_all_exported_symbols)
        {
            if (m_all_associated_symbols.find(name) == m_all_associated_symbols.end()) {
                log_error("Symbol or hit group '" + wstr_to_str(name) + 
        ...
        // Add a subobject for the association between shader and the payload
        ...
        // Associate the set of shader with the payload defined in the previous subobject
