...
namespace mdl_d3d12
{
        ...
        , m_iteration_count(iteration_count)
    {
        ...
        if(str_ends_with(m_file_path, ".exr") || str_ends_with(m_file_path, ".hdr"))
    ...
    int Window_image_file::show(int nCmdShow)
    {
            ...
            for (size_t i = 0; i < m_iteration_count; ++i)
            {
                ...
                if ((i + 1) % message_step == 0)
                    log_info("rendering completed to " + 
                             std::to_string((i + 1) / message_step * 10) + "%\r");
        ...
        if (m_back_buffer->download(tile->get_data()))
            m_app->get_mdl_sdk().get_compiler().export_canvas(m_file_path.c_str(), canvas.get());
